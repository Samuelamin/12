import React from 'react';
import { Link } from 'react-router-dom';
import Paypall from './Paypall';

export default function CartTotal({value , history}) {
    const {cartSubTotal , cartTax ,cartTotal ,clearCart} = value;

    return (
        <div>
          <React.Fragment>
          <div className='container'>
              <div className='row'>
                  <div className='col-10 mt-2 ml-sm-5 ml-md-auto col-sm-8
                  text-capitalize text-right'
                  style={{textAlign:"right" , marginLeft:"auto"}}>
                      <Link to='/'>
                      <button className='btn btn-outline-danger tex-uppercase
                      mb-3 px-5' type="button " onClick={()=>clearCart()}>
                          clear cart
                      </button>
                      </Link>
                      <h5>
                        <span className='text-title'>
                            subtotal :
                        </span> 
                         <strong> {cartSubTotal}EG </strong>
                      </h5>
                      <h5>
                        <span className='text-title'>
                            tax :
                        </span> 
                         <strong> {cartTax}EG </strong>
                      </h5>
                      <h5>
                        <span className='text-title'>
                            total :
                        </span> 
                         <strong> {cartTotal} EG </strong>
                      </h5>
                      <Paypall total={cartTotal} clearCart={clearCart} history={history} />
                  </div>
              </div>
          </div>
          </React.Fragment>  
        </div>
    )
}
